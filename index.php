...
<body>
        ...
        <section class="slider" id="slider">
            <div class="slider__window">
                <ul class="slider__list">
                    <? $slider_tours = mysqli_query($con, "SELECT * FROM `tours` WHERE `tours_slider` = 1");
                        if (mysqli_num_rows($slider_tours)){
                            while ($slide = mysqli_fetch_assoc($slider_tours)){?>
                                <li class="slider__item" style="background-image: url(images/tours/<?=$slide['tours_image']?>);">
                                    <div class="container">
                                        <div class="slider__content">
                                            <h2 class="title"><?=$slide['tours_title']?></h2>
                                            <p class="description"><?=$slide['tours_description']?></p>
                                        </div>
                                    </div>
                                </li>
                            <?}
        ...
        <section class="tours" id="tours">
            <div class="container">
                <ul class="tours__list">
                    <? $tours = mysqli_query($con, "SELECT * FROM `tours`");
                        if (mysqli_num_rows($tours)){
                            while ($tour = mysqli_fetch_assoc($tours)){?>
                                <li class="tours__item">
                                    <div class="tours__image">
                                        <img src="images/tours/<?=$tour['tours_image']?>" alt="<?=$tour['tours_title']?>" width="100%">
                                    </div>
                                    <div class="tours__text">
                                        <h2 class="title"><?=$tour['tours_title']?></h2>
                                        <div class="price"><?=$tour['tours_price']?></div>
                                        <p class="description"><?=$tour['tours_description']?></p>
        ...
        <section class="form" id="form">
            <div class="container">
                <form action="#">
                    ...
                    <label for="tour-id">Выберите тур:</label>
                    <select name="tour-id" id="tour-id">
                        <? $options = mysqli_query($con, "SELECT * FROM `tours`");
                            if (mysqli_num_rows($options)){
                                while ($option = mysqli_fetch_assoc($options)){?>
                                    <option value="<?=$option['tours_id']?>"><?=$option['tours_title']?></option>
                                <?}
                            }
                        ?>
                    </select>
    ...
    <footer id="footer" class="footer">
        <div class="container">
            ...
            <p class="about-description">
                Lorem ipsum, dolor sit amet consectetur adipisicing elit. Veritatis, officiis earum. Voluptatibus, maxime numquam? Ipsam!
            </p>
        </div>
        </div>
    </footer>
    ...
    <div class="modal">
        <div class="modal__card">
            <div class="modal__image">
                <img src="" alt="Tour image" width="100%">
            </div>
            <div class="modal__text">
                <div class="head">
                    <h2 class="title"></h2>
                    <div class="price">Цена: <span class="count"></span> руб.</div>
                </div>
    ...
    <script src="js/full-tour.js"></script>
</body>
</html>